...
export default function SterilizersScreen() {
  ...
  const handleAdd = async () => {
    ...
    setAdding(false);
    if (error) { Alert.alert('Помилка', error.message); return; }
    setNewName('');
    setNewBrand('');
    setNewType('dry_heat');
    setShowForm(false);
  ...
  const handleDelete = (id: string, name: string) => {
    Alert.alert('Видалити стерилізатор?', `"${name}" буде видалено назавжди.`, [
      ...
      { text: 'Видалити', style: 'destructive', onPress: async () => {
        await supabase.from('sterilizers').delete().eq('id', id);
        load();
      }},
    ]);
  };
  ...
  const renderItem = ({ item }: { item: SterilizerRow }) => {
        ...
        <View style={styles.cardContent}>
          ...
          <View style={styles.tagRow}>
            <View style={[styles.tag, { backgroundColor: typeStyle.bg }]}>
              <Text style={[styles.tagText, { color: typeStyle.color }]}>{typeStyle.label}</Text>
            </View>
            {item.brand ? (
              <View style={[styles.tag, { backgroundColor: COLORS.cardBg }]}>
                <Text style={[styles.tagText, { color: COLORS.textSecondary }]}>{item.brand}</Text>
              </View>
            ) : null}
          </View>
        </View>
          ...
          <View style={styles.typeRow}>
            {TYPE_OPTIONS.map((opt) => (
              <TouchableOpacity
                ...
                style={[
                  styles.typeChip,
                  newType === opt.value && { backgroundColor: opt.bg, borderColor: opt.color },
                ]}
              ...
              >
                ...
                <Text style={[styles.typeLabel, newType === opt.value && { color: opt.color, fontWeight: '600' }]}>
                  {opt.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <TouchableOpacity style={styles.submitBtn} onPress={handleAdd} activeOpacity={0.8} disabled={adding}>
            {adding ? (
              ...
              <>
                ...
                <Text style={styles.submitBtnText}>Додати стерилізатор</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
          ...
          ListHeaderComponent={
            items.length > 0 ? (
              <Text style={styles.listTitle}>
                {items.length} {items.length === 1 ? 'стерилізатор' : items.length < 5 ? 'стерилізатори' : 'стерилізаторів'}
              </Text>
            ) : null
          }
          ListEmptyComponent={
            <View style={styles.emptyState}>
              <View style={styles.emptyIcon}>
                <MaterialCommunityIcons name="radiator" size={36} color={COLORS.textSecondary} />
              </View>
              ...
              {!showForm && (
                <TouchableOpacity style={styles.emptyAddBtn} onPress={() => setShowForm(true)} activeOpacity={0.7}>
                  ...
                  <Text style={styles.emptyAddText}>Додати перший стерилізатор</Text>
                </TouchableOpacity>
              )}
            </View>
          }
