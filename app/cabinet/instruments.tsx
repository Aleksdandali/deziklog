...
import { View, FlatList, StyleSheet, SafeAreaView, TouchableOpacity, Alert, ActivityIndicator } from 'react-native';
...
export default function InstrumentsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .eq('user_id', userId)
      .order('created_at');
    if (error && __DEV__) console.error('Instruments error:', error.message);
    setItems(data ?? []);
    setLoading(false);
  }, [userId]);
  ...
  const handleDelete = (id: string, name: string) => {
    Alert.alert('Видалити інструмент?', `"${name}" буде видалено назавжди.`, [
      ...
      { text: 'Видалити', style: 'destructive', onPress: async () => {
        if (!userId) return;
        const { error } = await supabase.from('instruments').delete().eq('id', id).eq('user_id', userId);
        if (error && __DEV__) console.error('Delete instrument error:', error.message);
        load();
      }},
    ]);
  };
