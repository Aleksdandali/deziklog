  ...
  useFocusEffect(useCallback(() => {
    (async () => {
      ...
      const [profileRes, instrRes, sterRes] = await Promise.all([
        ...
        supabase.from('instruments').select('id', { count: 'exact', head: true }).eq('user_id', uid),
        supabase.from('sterilizers').select('id', { count: 'exact', head: true }).eq('user_id', uid),
      ]);
        ...
        <View style={styles.menuSection}>
          ...
          >
            <View style={[styles.menuIcon, { backgroundColor: '#FFF3E0' }]}>
              <MaterialCommunityIcons name="radiator" size={20} color="#E65100" />
            </View>
            <View style={styles.menuContent}>
              <Text style={styles.menuLabel}>Стерилізатори</Text>
              <Text style={styles.menuSub}>Обладнання для стерилізації</Text>
            </View>
            <View style={styles.menuRight}>
              <View style={styles.countBadge}>
                <Text style={styles.countText}>{counts.sterilizers}</Text>
              </View>
          ...
          >
            ...
            <View style={styles.menuContent}>
              <Text style={styles.menuLabel}>Інструменти</Text>
              <Text style={styles.menuSub}>Ножиці, пінцети, кусачки</Text>
            </View>
            <View style={styles.menuRight}>
              <View style={styles.countBadge}>
                <Text style={styles.countText}>{counts.instruments}</Text>
              </View>
              <Feather name="chevron-right" size={16} color={COLORS.textSecondary} />
            </View>
          </TouchableOpacity>
