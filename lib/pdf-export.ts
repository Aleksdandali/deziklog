...
export async function generateJournalPDF(
  ...
  const rows = cycles.map((c, i) => {
    const date = new Date(c.started_at).toLocaleDateString('uk-UA', {
      day: '2-digit', month: '2-digit', year: 'numeric',
    });
    const time = new Date(c.started_at).toLocaleTimeString('uk-UA', {
  ...
  const html = `
    ...
    <body>
      ...
      <div class="footer">
        Dezik Log · Цифровий журнал стерилізації
      </div>
    </body>
    </html>
  `;
